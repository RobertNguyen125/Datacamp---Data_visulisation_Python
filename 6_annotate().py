...
# compute the max enrollment  for women in computer_science
cs_max = computer_science.max()
# find out the year where for the max enrollment for computer_science
yr_max = year[computer_science.argmax()]
# plot with legend
plt.plot(year, computer_science, color='red', label='Computer Science')
plt.plot(year, physical_sciences, color='blue', label='Physical Sciences')
plt.legend(loc='lower right')
#add blackarrow annotation
plt.annotate('Maximum', xy=(yr_max, cs_max), xytext=(yr_max+5,cs_max+5), arrowprops={'facecolor':'black'})
plt.xlabel('Year')
plt.ylabel('Enrollment (%)')
plt.title('Undergraduate enrollment of women')
plt.show()
plt.savefig('/Users/apple/desktop/dataVisualisation/1_customisingPlots/6_annotate.png')
